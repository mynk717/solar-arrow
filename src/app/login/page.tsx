...
function LoginForm() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-600 via-indigo-600 to-purple-700 p-5">
      <style jsx global>{`
        @import url('https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;600;700&display=swap');
        * { font-family: 'Poppins', sans-serif; }
      `}</style>

      <div className={`auth-wrapper bg-white rounded-3xl shadow-2xl relative overflow-hidden w-[900px] max-w-full min-h-[600px] transition-all duration-600 ${isSignUp ? 'panel-active' : ''}`}>
        ...
        <div className={`login-form-box absolute top-0 h-full w-full lg:w-1/2 left-0 z-[2] transition-all duration-600 ${isSignUp ? 'lg:translate-x-full' : ''}`}>
          <div className="bg-white flex items-center justify-center flex-col px-8 lg:px-12 h-full text-center py-8 overflow-y-auto">
            {/* Logo */}
            <div className="mb-6 flex items-center gap-3">
              <div className="bg-blue-600 rounded-lg p-2 w-14 h-14 flex items-center justify-center">
                <img 
                  src="/SA_logo.png" 
                  alt="Logo" 
              ...
              <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">Solar Arrow</h1>
            </div>

            <h2 className="text-xl lg:text-2xl font-semibold mb-4 text-gray-800">Sign In</h2>

            {/* Login Type Toggle */}
            <div className="flex rounded-lg bg-gray-100 p-1 mb-4 w-full max-w-xs">
              <button
            ...
            {/* Error Message */}
            {error && (
              <div className="bg-red-50 border border-red-200 text-red-600 px-4 py-2 rounded-lg mb-4 text-sm w-full max-w-xs">
                {error}
              </div>
            )}
            ...
            {loginType === 'admin' && (
              <>
                <button
                  ...
                  disabled={loading}
                  className="w-full max-w-xs flex items-center justify-center gap-2 bg-white border-2 border-gray-300 text-gray-700 py-2.5 px-4 rounded-xl hover:bg-gray-50 hover:border-blue-500 transition-all disabled:opacity-50 mb-3"
                >
                  {loading ? (
            ...
            <form onSubmit={handleCredentialsSignIn} className="w-full max-w-xs">
              <div className="relative w-full mb-3">
                ...
                <input
                  ...
                  className="w-full pl-11 pr-4 py-2.5 bg-gray-100 border-2 border-transparent rounded-xl focus:border-blue-500 focus:bg-white transition-all outline-none text-sm text-gray-600"
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold py-2.5 px-8 rounded-full uppercase tracking-wider text-sm shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all disabled:opacity-50"
              >
        ...
        <div className={`register-form-box absolute top-0 h-full w-full lg:w-1/2 left-0 transition-all duration-600 ${isSignUp ? 'lg:translate-x-full opacity-100 z-[5]' : 'opacity-0 z-[1] lg:block hidden'}`}>
          <div className="bg-white flex items-center justify-center flex-col px-8 lg:px-12 h-full text-center py-8">
            <div className="mb-6 flex items-center gap-3">
              <div className="bg-blue-600 rounded-lg p-2 w-14 h-14 flex items-center justify-center">
                <img 
                  src="/SA_logo.png" 
            ...
            <h2 className="text-xl lg:text-2xl font-semibold mb-4 text-gray-800">Create Account</h2>
            ...
            <form onSubmit={handleSignup} className="w-full max-w-xs">
              ...
              <button
                type="submit"
                className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold py-2.5 px-8 rounded-full uppercase tracking-wider text-sm shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all"
              >
                Sign Up
              </button>
            </form>
        ...
        <div className={`hidden lg:block absolute top-0 left-1/2 w-1/2 h-full overflow-hidden transition-transform duration-600 z-[100] ${isSignUp ? '-translate-x-full' : ''}`}>
          <div className={`bg-gradient-to-br from-blue-600 via-indigo-600 to-purple-700 relative -left-full h-full w-[200%] transition-transform duration-600 ${isSignUp ? 'translate-x-1/2' : ''}`}>
            ...
            <div className={`absolute flex items-center justify-center flex-col px-10 text-center top-0 h-full w-1/2 right-0 text-white transition-transform duration-600 ${isSignUp ? 'translate-x-[20%]' : ''}`}>
              ...
              <p className="text-lg mb-8 opacity-90">Enter your credentials to access your Solar Arrow dashboard</p>
              <button
                onClick={() => setIsSignUp(true)}
                className="bg-transparent border-2 border-white text-white py-3 px-12 rounded-full uppercase tracking-wider hover:bg-white/10 transition-all"
              >
                Sign Up
              </button>
            </div>
            ...
            <div className={`absolute flex items-center justify-center flex-col px-10 text-center top-0 h-full w-1/2 left-0 text-white transition-transform duration-600 ${isSignUp ? '' : '-translate-x-[20%]'}`}>
              ...
              <p className="text-lg mb-8 opacity-90">Register with us and start managing your solar projects</p>
              <button
                onClick={() => setIsSignUp(false)}
                className="bg-transparent border-2 border-white text-white py-3 px-12 rounded-full uppercase tracking-wider hover:bg-white/10 transition-all"
              >
